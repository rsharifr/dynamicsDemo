...
OMEGA_AXIS     = 2       # Z axis from gyro CSV
CAMERA_INDEX   = 0       # 1 = external USB webcam
...
def on_gyro_notify(sender, data: bytearray):
    ...
    try:
        ...
        if len(vals) < 6:
            return
        omega_rads = -math.radians(vals[OMEGA_AXIS])
        r_m        = latest_r
        v_ms       = abs(omega_rads) * r_m
        with buf_lock:
...
def build_gui():
    ...
    # Panel 1: ω
    ax_omega.set_title("Angular Velocity  ω  (rad/sec)")
    ax_omega.set_xlabel("time (sec)"); ax_omega.set_ylabel("ω (rad/sec)")
    ...
    ax_r.set_xlim(-WINDOW_SECONDS, 0); ax_r.set_ylim(0.035, 0.235)
    ...
    ax_v.set_xlim(-WINDOW_SECONDS, 0); ax_v.set_ylim(0, 5)
    ...
    line_ax_plot, = ax_phase.plot(t, ax_buf, color="#E24B4A", linewidth=1.2, label="ax")
    line_ay_plot, = ax_phase.plot(t, ay_buf, color="#378ADD", linewidth=1.2, label="ay")
    ...
    def update_graphs():
        ...
        ax_v.set_ylim(0, max(vb.max() * 1.2, 0.01))
